...
local function paramcheck(_x)
	if _x and type(_x) ~= "number"  then error(debug.traceback("Rect",2)) end
end

---@class Rect

---@alias UnityEngine.Rect Rect

EC.Rect = {
	---@return Rect
	new = function(_x, _y, _width, _height)
			local ret = { x = _x , y = _y, width = _width, height = _height }
	...
	__eq = function(self,rhs)
			if math.abs( self.x - rhs.x) > 0.000001 then return false end
			if math.abs( self.y - rhs.y) > 0.000001 then return false end
			if math.abs( self.width - rhs.width) > 0.000001 then return false end
			if math.abs( self.height - rhs.height) > 0.000001 then return false end
			return true
	end,

}

EC.Rect.__index = function(t,k)
...
EC.Rect.__newindex = function(t,k,v)
  local mt = getmetatable(t)
  local setfunc = rawget(mt,"set_" .. k)
  if setfunc then setfunc(t,v) return t end
  return nil
end
