...
local function paramcheck(_x)
	if _x and type(_x) ~= "number"  then error(debug.traceback("EC.Vector4Check",2)) end
end

---@class Vector4

---@alias UnityEngine.Vector4 Vector4

EC.Vector4 = {
	---@return Vector4
	new = function (_x,_y,_z,_w)
		local ret = { x = _x and _x or 0 , y = _y and _y or 0 , z = _z and _z or 0, w = _w and _w or 0 };
		setmetatable(ret,EC.Vector4)
		return ret
	end,

	__add = function( lhs, rhs)
		local ret = { x = lhs.x + rhs.x, y = lhs.y + rhs.y, z = lhs.z + rhs.z , w = lhs.w + rhs.w }
		setmetatable(ret, EC.Vector4)
		return ret
	end,
	...
	__mul = function( self, factor)
		local ret = { x = self.x * factor, y = self.y * factor, z = self.z * factor , w = self.w * factor}
		setmetatable(ret, EC.Vector4)
		return ret
	end,
	...
	__unm = function( self )
		local ret = { x = -self.x, y = -self.y, z = -self.z, w = -self.w}
		setmetatable(ret,EC.Vector4)
		return ret
	end,
	...
	__eq = function( lhs, rhs)
		...
		if math.abs( lhs.z - rhs.z) > 0.000001 then return false end
		if math.abs( lhs.w - rhs.w) > 0.000001 then return false end
		return true
	end,
	...
	Normalize = function( self )
		...
		self.x = self.x / len; self.y = self.y / len; self.z = self.z / len; self.w = self.w / len;
	end,
...
EC.Vector4.__index = function(t,k)
